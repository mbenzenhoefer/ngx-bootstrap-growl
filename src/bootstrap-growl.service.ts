...
export class BootstrapGrowlService {
    ...
    alertCount: number = 999;
    autoClose: number = -1;

    constructor() { }

    configure(alertCount: number, autoClose: number){
    ...
    addAlert(message: string, type: BootstrapAlertType, dismissable?: boolean): void{
    ...
    private _scheduleAlertHide(timeout: number, alert: BootstrapAlert) {
        var displayTimeout = Observable.timer(timeout);
